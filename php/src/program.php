...
<body>
    ...
    <main class="main">
        ...
        <div class="aranzman-main">
            ...
            <?php 
            $polazak=$row["termin_polazak"];
            $datum= date("Y-m-d");
            if($polazak > $datum){
                echo '<a href="rezervacija.php?pon='.$idpon.'&lok='.$lok.'" ><button class="button-rezervacija">Резервиши</button></a>';
            }
            else{
                echo '<button class="button-rezervacija">Понуда истекла</button>';
            }
            ?>

                
                <?php 
                if(isset($_SESSION["id_tipa"])){
                if($_SESSION['id_tipa'] == 2 || $_SESSION['id_tipa'] == 3){
